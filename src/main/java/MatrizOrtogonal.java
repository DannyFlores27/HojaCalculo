...
public class MatrizOrtogonal implements Serializable {
    ...
    public static MatrizOrtogonal cargarDesdeArchivo(String nombreArchivo) throws IOException, ClassNotFoundException {
        ObjectInputStream ois = null;
        MatrizOrtogonal matrizCargada = null;

        try {
            ois = new ObjectInputStream(new FileInputStream(nombreArchivo));
            matrizCargada = (MatrizOrtogonal) ois.readObject();
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            try {
